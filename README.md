...
目标体验是：用户从 GitHub 下载 App 后可以直接打开；如果本机缺少 NTFS 读写组件，App 会打开安装窗口引导安装。安装过程中 macOS 可能要求用户在“系统设置 > 隐私与安全性”允许读写组件，并可能需要重启，这一步不能被普通 App 静默绕过。

## 功能

- 扫描本机外接或内置 NTFS 分区
- 显示容量、设备路径、当前挂载点和读写状态
- 检查 Homebrew 和 NTFS 读写组件是否可用
- 在应用内打开读写组件安装窗口
...
3. 如果 macOS 提示允许 NTFS 读写组件，请到“系统设置 > 隐私与安全性”中允许，并按提示重启 Mac。
...
如果安装读写组件后 macOS 提示需要允许系统软件，请按系统提示到“系统设置 > 隐私与安全性”中允许，必要时重启一次。
...
- 当前可以引导安装依赖，但不能自动替用户完成 NTFS 读写组件的系统安全确认；这一步必须由用户在系统设置中确认。
- 未做签名、公证、自动更新、崩溃上报。
